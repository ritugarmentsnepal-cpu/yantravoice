...
<div class="mobile-wrapper">
    ...
    <main class="flex-1 px-6 py-2 pb-8">
        ...
        <div id="pane-dashboard" class="tab-pane block space-y-4 pb-6">
            ...
            {{-- NEW AI UGC BANNER --}}
            <a href="{{ route('ugc.index') }}" class="block relative overflow-hidden bg-gradient-to-r from-coral to-orange-500 rounded-2xl p-5 shadow-lg shadow-orange-500/30 group active:scale-[0.98] transition-transform mt-3">
                ...
                <div class="relative z-10 flex items-center justify-between">
                    <div>
                        <span class="inline-block px-2 py-0.5 bg-white text-coral text-[9px] font-extrabold rounded-full uppercase tracking-wider mb-2 shadow-sm">New Feature</span>
                        <h3 class="text-white font-extrabold text-lg leading-tight">AI UGC Short Generator <span class="text-white/80 text-sm font-medium">(Beta)</span></h3>
                        <p class="text-white/80 text-xs font-medium mt-1">Create highly-retentive transparent actor videos instantly.</p>
                    </div>
                    <div class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center text-white backdrop-blur-sm group-hover:bg-white group-hover:text-coral transition-colors">
                        ➔
                    </div>
                </div>
            </a>
    ...
    <div id="buyCreditsPopup" class="hidden fixed inset-0 z-[90] bg-black/50 backdrop-blur-sm flex justify-center">
        ...
        <div id="pane-voiceover" class="tab-pane hidden space-y-5">
            ...
            {{-- Toggles --}}
            <div class="flex bg-[#F0F4F8] p-1.5 rounded-full mb-5">
                <button id="voModeTextBtn" class="flex-1 py-2 text-[13px] font-bold rounded-full bg-white text-navy shadow-sm transition-all text-center">Write Script</button>
                <button id="voModeVideoBtn" class="flex-1 py-2 text-[13px] font-medium rounded-full text-[#64748B] hover:text-navy transition-all text-center">Upload Video</button>
            </div>

            {{-- Step 1: Upload Video (Hidden by default) --}}
            <div id="voModeVideo" class="hidden space-y-5">
                <div>
                    <label class="block text-sm font-bold text-navy mb-2">Upload Video</label>
                    <div class="border-2 border-dashed border-[#C0C9D6] rounded-2xl p-6 text-center hover:bg-[#F8FAFC] transition-colors cursor-pointer" id="voVideoDropzone" onclick="document.getElementById('voVideoUploadInput').click()">
                        <input type="file" id="voVideoUploadInput" class="hidden" accept="video/mp4,video/quicktime,video/x-msvideo,video/webm">
                        <div class="text-3xl mb-2">📥</div>
                        <p class="text-sm font-bold text-navy mb-1" id="voVideoFileName">Click to browse or drag video here</p>
                        <p class="text-[10px] text-[#94A3B8]">MP4, MOV, WEBM up to 100MB (Max 5 mins)</p>
                    </div>
                </div>
                ...
                <div class="pt-1">
                    <button type="button" id="voAnalyzeBtn"
                        class="w-full py-4 bg-navy hover:bg-[#2A3548] text-white font-bold text-sm rounded-full transition-all active:scale-[0.98] shadow-lg shadow-navy/20 flex justify-center items-center gap-2 btn-progress">
                        <span class="progress-fill"></span>
                        <span class="progress-text">📹 Analyze Video & Generate Script</span>
                    </button>
                </div>
                <p class="text-[10px] text-center text-[#94A3B8]">💡 AI will analyze your video and write a script for you to voiceover</p>
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Toggle Modes
        const voModeTextBtn = document.getElementById('voModeTextBtn');
        const voModeVideoBtn = document.getElementById('voModeVideoBtn');
        const voModeText = document.getElementById('voModeText');
        const voModeVideo = document.getElementById('voModeVideo');
        
        voModeTextBtn.addEventListener('click', () => {
            voModeTextBtn.className = 'flex-1 py-2 text-[13px] font-bold rounded-full bg-white text-navy shadow-sm transition-all text-center';
            voModeVideoBtn.className = 'flex-1 py-2 text-[13px] font-medium rounded-full text-[#64748B] hover:text-navy transition-all text-center';
            voModeText.classList.remove('hidden');
            voModeVideo.classList.add('hidden');
        });

        voModeVideoBtn.addEventListener('click', () => {
            voModeVideoBtn.className = 'flex-1 py-2 text-[13px] font-bold rounded-full bg-white text-navy shadow-sm transition-all text-center';
            voModeTextBtn.className = 'flex-1 py-2 text-[13px] font-medium rounded-full text-[#64748B] hover:text-navy transition-all text-center';
            voModeVideo.classList.remove('hidden');
            voModeText.classList.add('hidden');
        });

        // Video Upload File Selection
        const voVideoUploadInput = document.getElementById('voVideoUploadInput');
        const voVideoFileName = document.getElementById('voVideoFileName');
        voVideoUploadInput.addEventListener('change', function() {
            if(this.files.length > 0) {
                voVideoFileName.textContent = this.files[0].name;
            } else {
                voVideoFileName.textContent = 'Click to browse or drag video here';
            }
        });

        // Analyze Video
        document.getElementById('voAnalyzeBtn').addEventListener('click', async function() {
            const file = voVideoUploadInput.files[0];
            if(!file) return showToast('Please select a video file first.');
            
            const btn = this;
            const origHtml = btn.innerHTML;
            const progress = startBtnProgress(btn, 'Analyzing Video', 20000);
            ...
            try {
                ...
                data.script.forEach(scene => {
                    fullScript += scene.text + " ";
                });
                
                // Put script into the text input area
                textInput.value = fullScript.trim();
                textInput.dispatchEvent(new Event('input')); // trigger char counter update
                
                showToast('Script generated! You can edit it below.', 'success');
                
                // Switch back to text mode so user can see it
                voModeTextBtn.click();
                
                // Scroll down to text area
        ...
        generateBtn.addEventListener('click', async function (e) {
            ...
            const emotion  = emotionInput.value.trim() || 'Neutral';
        ...
        window.renderScriptSegments = function renderScriptSegments(scriptData) {
            ...
            container.innerHTML = segments.map((seg, i) => {
                const clipIdx = seg.clip_index !== undefined ? seg.clip_index : 0;
                const startSec = Math.floor(seg.start_time || seg.start || 0);
                const endSec = Math.floor(seg.end_time || seg.end || 0);
                const timeLabel = `Clip ${clipIdx + 1} • ${startSec}s – ${endSec}s`;
                
                const voText = seg.voiceover_text || seg.text || '';
                const txtOverlay = seg.text_overlay || '';
                const trans = seg.transition || 'none';
                
                const transitions = ['none', 'fade', 'wipeleft', 'wiperight', 'slideup', 'slidedown', 'pixelize'];
                const transOptions = transitions.map(t => `<option value="${t}" ${trans === t ? 'selected' : ''}>${t}</option>`).join('');

                return `<div class="bg-white rounded-xl border border-[#E8EDF2] p-4 shadow-sm hover:border-coral/40 transition-colors mb-3">
                    <div class="flex items-center justify-between mb-3 border-b border-[#E8EDF2] pb-2">
                        <div class="flex items-center gap-2">
                            <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-coral/10 text-coral text-[10px] font-bold">#${i+1}</span>
                            <span class="text-xs font-bold text-navy tracking-wide">${timeLabel}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Transition</span>
                            <select class="seg-transition bg-[#F8FAFC] border border-[#E8EDF2] rounded-lg text-[10px] p-1 font-bold text-navy focus:ring-coral/30 cursor-pointer">
                                ${transOptions}
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <div>
                            <label class="block text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider mb-1">Voiceover Script</label>
                            <textarea data-seg-index="${i}" data-seg-clip="${clipIdx}" data-seg-start="${startSec}" data-seg-end="${endSec}" rows="2"
                                class="seg-text w-full bg-[#F8FAFC] border border-[#E8EDF2] rounded-lg p-2.5 text-navy text-xs font-medium leading-relaxed focus:ring-1 focus:ring-coral/30 focus:border-coral/30 resize-none">${voText}</textarea>
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider mb-1">Text Overlay (On Screen)</label>
                            <input type="text" class="seg-overlay w-full bg-[#F8FAFC] border border-[#E8EDF2] rounded-lg p-2 text-navy text-xs font-bold focus:ring-1 focus:ring-coral/30 focus:border-coral/30" value="${txtOverlay}" placeholder="Optional short text...">
                        </div>
                    </div>
                </div>`;
            }).join('');
        ...
        function collectScriptSegments() {
            const containers = document.querySelectorAll('#adScriptSegments > div');
            ...
            containers.forEach(div => {
                const ta = div.querySelector('.seg-text');
                const ov = div.querySelector('.seg-overlay');
                const tr = div.querySelector('.seg-transition');
                
                if (ta) {
                    segments.push({
                        clip_index: parseInt(ta.dataset.segClip || 0),
                        start_time: parseFloat(ta.dataset.segStart || 0),
                        end_time: parseFloat(ta.dataset.segEnd || 0),
                        voiceover_text: ta.value.trim(),
                        text_overlay: ov ? ov.value.trim() : '',
                        transition: tr ? tr.value : 'none'
                    });
                }
            });
