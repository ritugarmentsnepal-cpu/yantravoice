...
class ApiSetting extends Model
{
    ...
    /**
     * Get the admin's HeyGen API key.
     */
    public static function getHeyGenApiKey(): ?string
    {
        $key = static::getValue('heygen_api_key');
        return $key && strlen($key) > 5 ? $key : null;
    }

    /**
     * Get the admin's Pexels API key.
     */
    public static function getPexelsApiKey(): ?string
    {
        $key = static::getValue('pexels_api_key');
        return $key && strlen($key) > 5 ? $key : null;
    }
